...
export async function PUT(
  ...
  try {
    ...
    if (packageSpecifier !== undefined) { fields.push(`package_specifier = $${paramIndex++}`); values.push(packageSpecifier) }
    if (config !== undefined) { fields.push(`config = $${paramIndex++}`); values.push(JSON.stringify(config)) }
